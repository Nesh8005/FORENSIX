...
# Install apt packages (volatility3 removed as it's often better via pip/git)
sudo apt-get install -y \
    ...
    wireshark-common \
    tshark \
    sleuthkit || echo "Warning: Some apt packages failed. Continuing..."

# 3. Create Virtual Environment
echo -e "\033[1;34m[*] Creating Python virtual environment...\033[0m"
# Always try to create venv, even if folder exists (to be safe)
python3 -m venv venv || echo "Error creating venv. Is python3-venv installed?"

# Check if venv was actually created
if [ ! -f "venv/bin/activate" ]; then
    echo -e "\033[1;31m[!] CRITICAL: venv creation failed.\033[0m"
    echo "    Try running manually: sudo apt install python3-venv && python3 -m venv venv"
    exit 1
fi
